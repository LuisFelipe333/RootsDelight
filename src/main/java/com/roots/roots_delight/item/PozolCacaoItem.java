...
public class PozolCacaoItem extends Item {
    public PozolCacaoItem(Properties properties) {
        super(properties);
    }
    ...
    public ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity entity) {
        ...
        if (!level.isClientSide) {
            float chance = 0.15f; // Default fallback

            try {
                // Try to get the probability from your TOML config
                chance = PozolConfig.POZOL_CACAO_CHANCE.get().floatValue();
            } catch (IllegalStateException e) {
                // If the config isn't loaded yet, it uses the 0.15f above
            }
            ...
            applyEffect(entity, MobEffects.SLOW_FALLING, 1200, 0, chance, rand);
